...
- **Cross-pollination index** (`src/cross_pollination_index.py`) — detects when a solution from one project appears in another; Jaccard similarity over memory content; `detect_cross_pollination(memories_by_project)` compares every project pair; `get_project_similarity_matrix()` aggregates events per pair; `get_most_transferred_knowledge()` surfaces frequently reused memories; `format_summary()` renders markdown brief. 30 tests
- **FSRS context-relevance weighting** (`src/fsrs_context_weighting.py`) — combines FSRS due-urgency score (40%) with content-based Jaccard context relevance (60%); `weight_memories(memories, query, states, weights, top_k)` returns `WeightedMemory` objects sorted by combined_score; custom weights supported; memories without FSRS state get neutral score; proves that a highly relevant but not-yet-due memory beats an irrelevant overdue one. 25 tests
...
- **Skill evolution tracker** (`src/skill_evolution.py`) — snapshots SKILL.md content per skill, stores unified diffs, and classifies each change as initial / meaningful / cosmetic / unchanged / missing. Standalone SQLite table (`skill_evolution_snapshots`). `SkillEvolutionTracker.snapshot()`, `snapshot_all()`, `get_history()`, `get_last_meaningful_update()`, `has_changed()`, `get_skills_by_change_type()`. 35 tests
- **Skill provenance tracker** (`src/skill_provenance.py`) — records every skill invocation with session ID, outcome, context snippet, and notes. `SkillProvenanceTracker.record_invocation()`, `get_history()`, `get_first_use()`, `get_sessions_for_skill()`, `get_skills_for_session()`, `get_co_invocations()`, `outcome_summary()`, `invocation_count()`. 42 tests
